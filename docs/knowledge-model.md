...
Contribution strength is a source-constrained metric for person-to-project membership. A documented membership begins with a low baseline, receives additional weight for its recorded tenure relative to other documented tenures in that project, and receives additional weight only for release groups that explicitly credit both the person and the project. Membership alone never implies that a person performed on every project release. Unknown dates and unavailable individual release credits remain unknown rather than being estimated.
