...
const claimValues = (entity, property) => (entity.claims?.[property] || []).map((claim) => claim.mainsnak?.datavalue?.value).filter(Boolean);

async function request(parameters) {
  const remaining = pauseMs - (Date.now() - lastRequestAt);
  if (remaining > 0) await sleep(remaining);
  const url = new URL(api);
  Object.entries({ action: "wbsearchentities", format: "json", language: "en", origin: "*", ...parameters }).forEach(([key, value]) => url.searchParams.set(key, value));
  ...
  if (!response.ok) throw new Error(`Wikidata API returned ${response.status} ${response.statusText}`);
...
for (const node of batch) {
  ...
  const match = (result.search || []).find((item) => normalize(item.label || "") === normalize(node.label));
  if (match) candidates.push({ node, qid: match.id });
}

const entities = new Map();
for (const group of chunks(candidates, 40)) {
  const result = await request({ action: "wbgetentities", ids: group.map((item) => item.qid).join("|"), props: "claims" });
  Object.entries(result.entities || {}).forEach(([id, entity]) => entities.set(id, entity));
}
...
for (const { node, qid } of candidates) {
  ...
  const property = node.musicbrainz.entity === "release-group" ? "P436" : "P434";
  if (!claimValues(entity, property).includes(node.musicbrainz.id)) {
    skipped.push({ id: node.id, label: node.label, candidate: qid, reason: `Candidate does not carry the exact ${property} MusicBrainz identifier` });
    continue;
  }
  const url = `https://www.wikidata.org/wiki/${qid}`;
  node.wikidata = { id: qid, url };
  addProvenance(node, `Wikidata: ${node.label}`, url, "Cross-reference accepted only after its MusicBrainz identifier exactly matched the graph identity.");
  const discogsArtistId = claimValues(entity, "P1953")[0];
  if (discogsArtistId && node.type !== "release") {
    node.discogs = { artistId: discogsArtistId, url: `https://www.discogs.com/artist/${discogsArtistId}` };
    addProvenance(node, `Discogs artist entry: ${node.label}`, node.discogs.url, "Catalog cross-reference supplied by Wikidata; Discogs content is not imported by this script.");
  }
...
for (const { node: person, entity } of mappings.values()) {
  ...
  for (const projectQid of claimValues(entity, "P463")) {
    ...
    const exists = graph.edges.some((edge) => edge.source === person.id && edge.target === project.id && edge.type === "member_of");
    if (exists) continue;
    const url = `https://www.wikidata.org/wiki/${person.wikidata.id}`;
    graph.edges.push({ source: person.id, target: project.id, type: "member_of", roles: [], sourceStatus: "verified", provenance: [{ title: `Wikidata membership claim: ${person.label} → ${project.label}`, url, note: "Exact mapped Wikidata entities state that the person is a member of the project." }] });
...
console.log(`Mapped ${mappings.size} exact Wikidata entities and added ${importedRelationships.length} relationships.`);
