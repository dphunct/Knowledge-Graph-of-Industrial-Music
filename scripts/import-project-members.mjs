...
function addProvenance(edge, item) {
  edge.provenance ||= [];
  if (!edge.provenance.some((sourceItem) => sourceItem.url === item.url)) {
    edge.provenance.push(item);
  }
}

function mergeMembershipFacts(target, duplicate) {
  target.roles = [...new Set([...(target.roles || []), ...(duplicate.roles || [])])];
  for (const item of duplicate.provenance || []) addProvenance(target, item);
...
for (const project of batch) {
  ...
  for (const relation of members) {
    ...
    const matchingMemberships = graph.edges.filter((edge) => edge.source === person.id && edge.target === project.id && edge.type === "member_of");
    const exactMembership = matchingMemberships.find((edge) => edge.validFrom === validFrom && edge.validTo === validTo);
    // A seed fact can predate its source research. Merge an exact roster result
    // into that undated fact so the map gains evidence without duplicate lines.
    const undatedMemberships = matchingMemberships.filter((edge) => !edge.validFrom && !edge.validTo);
    // A prior importer version could create a dated edge beside an undated
    // seed edge. Fold that placeholder into the exact roster record and keep
    // its role detail, leaving one relationship line for the same tenure.
    if (exactMembership && undatedMemberships.length) {
      for (const duplicate of undatedMemberships) {
        mergeMembershipFacts(exactMembership, duplicate);
        graph.edges.splice(graph.edges.indexOf(duplicate), 1);
      }
    }
    const undatedMembership = undatedMemberships[0];
    const membership = exactMembership || undatedMembership;
    const citation = source(project, member, relation);
    if (membership) {
      if (!membership.validFrom && validFrom) membership.validFrom = validFrom;
      if (!membership.validTo && validTo) membership.validTo = validTo;
      membership.sourceStatus = "verified";
      addProvenance(membership, citation);
    } else {
      graph.edges.push({ source: person.id, target: project.id, type: "member_of", roles: [], ...(validFrom ? { validFrom } : {}), ...(validTo ? { validTo } : {}), sourceStatus: "verified", provenance: [citation] });
    }
  }
}
