...
for (const node of graph.nodes) {
  ...
  if (node.musicbrainz && (!node.musicbrainz.id || !/^https:\/\/musicbrainz\.org\//.test(node.musicbrainz.url || ""))) throw new Error(`Invalid MusicBrainz identity: ${node.id}`);
  for (const source of node.provenance || []) {
    if (!source.title || !/^https:\/\//.test(source.url || "")) throw new Error(`Invalid node provenance: ${node.id}`);
  }
