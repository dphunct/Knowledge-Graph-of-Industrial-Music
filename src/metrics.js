...
export function graphMetrics(nodes, edges) {
  ...
  for (const edge of edges.filter((edge) => edge.type === "member_of" && nodeById.get(edge.source)?.type === "person" && nodeById.get(edge.target)?.type === "project")) {
  ...
  const overlapCount = (left = new Set(), right = new Set()) => [...left].filter((id) => right.has(id)).length;
  const membershipWeight = (edge) => {
    const peers = membershipsByProject.get(edge.target) || [];
    const duration = edge.validFrom && edge.validTo ? Math.max(1, edge.validTo - edge.validFrom + 1) : 0;
    const longestTenure = Math.max(...peers.map((item) => item.validFrom && item.validTo ? Math.max(1, item.validTo - item.validFrom + 1) : 0), 0);
    const tenureShare = longestTenure ? duration / longestTenure : 0;
    const projectReleases = releaseSets.get(edge.target) || new Set();
    const releaseShare = projectReleases.size ? overlapCount(releaseSets.get(edge.source), projectReleases) / projectReleases.size : 0;
    ...
    return .2 + .5 * tenureShare + .3 * releaseShare;
  };
  ...
  for (const edge of edges) {
    ...
    const weight = edge.type === "member_of" ? membershipWeight(edge)
      : edge.type === "released" ? 1 / Math.max(releaseSets.get(edge.source)?.size || 0, 1)
        : Number(edge.weight || 1);
    weightedLinks.get(edge.source).set(edge.target, (weightedLinks.get(edge.source).get(edge.target) || 0) + weight);
    weightedLinks.get(edge.target).set(edge.source, (weightedLinks.get(edge.target).get(edge.source) || 0) + weight);
  }
  const degreeValues = ids.map((id) => links.get(id).size);
  const contributionValues = ids.map((id) => [...weightedLinks.get(id).values()].reduce((sum, weight) => sum + weight, 0));
  ...
  for (let step = 0; step < 40; step += 1) {
    ...
    ids.forEach((id, from) => {
      ...
      weightedLinks.get(id).forEach((weight, to) => { next[indexById.get(to)] += .85 * rank[from] * weight / outgoing; });
    });
    rank.splice(0, rank.length, ...next);
  }
  const maxRank = Math.max(...rank, 1e-9);
  const bridge = ids.map((id) => [...weightedLinks.get(id)].reduce((sum, [neighbor, weight]) => sum + weight * weightedLinks.get(neighbor).size, 0));
  ...
  return new Map(ids.map((id, index) => {
    ...
    return [id, { degree, contribution, pageRank, betweenness, composite: (contribution + betweenness + pageRank) / 3 }];
  }));
}
